...
# Fix User= in service file to match the actual logged-in user (not always 'pi')
INSTALL_USER=$(whoami)
sudo sed -i "s/^User=.*/User=${INSTALL_USER}/" /etc/systemd/system/koe-amp.service
sudo systemctl daemon-reload
sudo systemctl restart koe-amp
